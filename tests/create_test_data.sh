...
# Script to create test NetCDF data files for ESMF_IO testing
# This script will try multiple approaches to create the test data

echo "Creating test data directory..."
mkdir -p tests/data

echo "Attempting to create test data files..."

# Check if we have Python with netCDF4 available in the Spack environment
if source /Users/barry/spack/var/spack/environments/my-esmf-env/.spack-env/view/bin/activate 2>/dev/null && python3 -c "import netCDF4, numpy" &> /dev/null; then
    echo "Creating test data using Python from Spack environment..."
    python3 << 'EOF'
import numpy as np
from netCDF4 import Dataset
import os

def create_test_input_data(filename, nx=20, ny=20, time_steps=1):
    """
...
def create_test_output_data(filename, nx=20, ny=20, time_steps=1, scale_factor=2.0):
    """
    ...
    with Dataset(filename, 'w', format='NETCDF4') as ncfile:
        ...
        # Create time variable
        time = ncfile.createVariable('time', 'f8', ('time',))
        time.units = 'hours since 200-01-01 0:00:00'
        time[:] = np.arange(time_steps)
...
EOF
    ...
    echo "Python with netCDF4 not available in Spack environment. Creating simple placeholder files using system Python..."
    
    # Create empty files as placeholders if Python is not available
    touch tests/data/input_test.nc
    touch tests/data/expected_output.nc
    echo "Created placeholder files. Please install netCDF4 to generate actual test data."
fi

echo "Test data creation process completed."