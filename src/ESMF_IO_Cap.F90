...
contains
  ...
  subroutine Realize(model, rc)
    ...
    ! Create importable fields for input data
    ...
    ! Create exportable fields for output data
...
!> Main program for the NUOPC ESMF_IO component
program main
  use ESMF
  use ESMF_IO_NUOPC
  ...
  write(error_msg, '(A,I0,A,I0,A)') "ESMF_IO NUOPC Driver starting on PE ", &
  ...
  model = ESMF_GridCompCreate(name="ESMF_IO_NUOPC", rc=rc)
  if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU)) then
  ...
  write(error_msg, '(A,I0,A,I0,A)') "ESMF_IO NUOPC Driver completed on PE ", &
                                    localPet, " of ", petCount, " PEs"
  call ESMF_LogWrite(trim(error_msg), ESMF_LOGMSG_INFO, rc=rc)
  if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU)) then
